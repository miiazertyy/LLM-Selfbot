...
REM Kill any lingering Python processes holding venv files open
echo Killing any lingering Python processes...
taskkill /f /im python.exe >nul 2>&1
taskkill /f /im pythonw.exe >nul 2>&1
timeout /t 2 /nobreak > nul

REM Clean up the update flag so the bot doesn't re-trigger on restart
...
:delete_retry
rmdir /s /q "%~dp0..\bot-env" 2>nul
if exist "%~dp0..\bot-env" (
...
call "%~dp0..\bot-env\Scripts\activate.bat"
...
start "LLMSelfbot" cmd /k "cd /d "%~dp0.." && call "%~dp0..\scripts\run.bat""
