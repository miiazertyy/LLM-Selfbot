...
from app.utils.helpers import load_instructions, load_config, resource_path
from app.utils.logger import log_system, log_error
...
def _my_account_index() -> int:
    """Which account this process drives (1-based), from the child env var."""
    try:
        return max(1, int(os.getenv("DISCORD_ACCOUNT_INDEX", "1") or "1"))
    except ValueError:
        return 1
...
class Management(commands.Cog):
    ...
    async def cog_before_invoke(self, ctx):
        # Human-like delay for non-owner users
    ...
    async def reload(self, ctx):
        if ctx.author.id == self.bot.owner_id:
            import sys as _sys
            cogs_dir = os.path.join(getattr(_sys, "_MEIPASS", os.path.abspath(".")), "app", "cogs")
            ...
            for filename in os.listdir(cogs_dir):
                if filename.endswith(".py"):
                    try:
                        await self.bot.unload_extension(f"app.cogs.{filename[:-3]}")
                        await self.bot.load_extension(f"app.cogs.{filename[:-3]}")
                    except Exception as e:
    ...
    async def update(self, ctx, source: str = "release"):
        ...
        if source not in ("release", "main"):
            await ctx.send("Invalid option. Use `,update` for latest release or `,update main` for latest commit.", delete_after=10)
            return
        ...
        if sys.platform == "win32":
            updater_path = os.path.join(repo_dir, "scripts", "updater.bat")
            # Use start with explicit /d so the bat file always runs from the repo root.
            subprocess.Popen(
                f'cmd /c start "Updating LLMSelfbot..." /d "{repo_dir}" /wait "{updater_path}" {source}',
                shell=True,
                cwd=repo_dir,
                creationflags=subprocess.CREATE_NEW_CONSOLE,
            )
        else:
            updater_path = os.path.join(repo_dir, "scripts", "updater.sh")
            try:
    ...
    async def config_cmd(self, ctx, key: str = None, *, value: str = None):
        ...
        if key is None:
            ...
            lines = [
                ...
                "─────────────────────────────",
                "Use `,config <key> <value>` to edit. Example: `,config tts.voice diana`",
            ]
    ...
    async def mood_cmd(self, ctx, *, mood_name: str = None):
        ...
        from app.utils.mood import get_mood
        config = load_config()
        ...
        import app.utils.mood as mood_module
    ...
    async def status(self, ctx, emoji: str = None, *, text: str = None):
        ...
        try:
            ...
            if text or emoji:
                await ctx.send("Status updated.", delete_after=10)
            else:
    ...
    async def _connect_and_keep_alive(self, target: discord.VoiceChannel):
        ...
        # Fatal close codes: 4006 session invalid, 4014 disconnected, 4017 DAVE
    ...
    async def image(self, ctx, action: str = None, *, name: str = None):
        ...
        if action is None:
            if invoked in ("imagels", "imagelist"):
                ...
                action = "ls"
        # Alias + no name: the typed arg IS the name
        ...
        if action in ("ls", "list"):
            ...
            from app.utils.helpers import load_config as _load_cfg
            import base64
            ...
            # Batch: split on commas / spaces (e.g. "1, 2, 3" or "1 2 3")
            ...
            await ctx.send("Usage: `,image ls` | `,image upload` | `,image download <n>` | `,image delete <n>`", delete_after=15)
    ...
    async def addfriend(self, ctx, user_id: int = None):
        ...
        if not user_id:
            ...
            return
        try:
            from app.utils.session import build_session
    ...
    async def clear(self, ctx, limit: int = 100):
        ...
        for msg in to_delete:
            ...
            try:
                ...
                failed += 1
        # Confirmation that self-deletes
        await ctx.send(
