...
def _ensure_telegram_installed():
    ...
    if getattr(sys, "frozen", False):
        # No pip inside a frozen app, telegram must be bundled.
        print("[TG Controller] python-telegram-bot is not bundled, reinstall the app.")
        sys.exit(1)
...
from app.core.ipc import (
    cmd_file as _cmd_file,
    result_file as _result_file,
    send_command as _send_command,
    wait_for_result as _wait_for_result,
    is_snap as _is_snap,
    snap_index as _snap_index,
)

_CONFIG_DIR = DATA_DIR / "config"
_IPC_DIR = DATA_DIR / "ipc"
...
TG_OWNER_ID = real_int("TELEGRAM_OWNER_ID")

if not TG_TOKEN or not TG_OWNER_ID:
    print(
        "\n[TG Controller] TELEGRAM_BOT_TOKEN / TELEGRAM_OWNER_ID are not set in\n"
        "config/.env, or still hold the template placeholders. Telegram control\n"
        "is off; everything else runs normally.\n",
        flush=True,
    )
    sys.exit(0)


# ── Discover how many accounts are configured ─────────────────────────────────
...
def _count_snap_accounts() -> int:
    """Account 1 = SNAP_USERNAME / SNAP_USERNAME_1; then _2, _3 contiguously."""
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not result:
        await update.message.reply_text(
            f"{label}⚠️ LLMSelfbot didn't respond. Make sure the IPC bridge is running in main.py."
        )
        return
...
async def cmd_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not result:
        await wait_msg.edit_text(f"{label}⚠️ LLMSelfbot didn't respond in time\\.", parse_mode=ParseMode.MARKDOWN_V2)
        return
...
async def _leaderboard_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await _wait_for_result(account, cmd_id, timeout=15.0)
    if not result:
        await query.edit_message_text("⚠️ LLMSelfbot didn't respond in time.")
        return
...
async def cmd_imagels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        def _build_nav(index: int):
            ...
            buttons.append(InlineKeyboardButton("🗑 Delete", callback_data=f"imgdel:{index}"))
            return InlineKeyboardMarkup([buttons]) if buttons else None
...
async def cmd_imagedelete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(nums) == 1:
        ...
        if result and result.get("ok"):
            ...
            await update.message.reply_text(f"{label}⚠️ LLMSelfbot did not respond in time.")
    else:
        ...
        if result and result.get("ok"):
            ...
            await status.edit_text(f"{label}⚠️ LLMSelfbot did not respond in time.")
...
async def cmd_wipe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result:
        ...
        await update.message.reply_text(f"{label}⚠️ Command sent. LLMSelfbot will wipe on next poll.")
...
async def cmd_persona(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rest.strip().lower() == "show":
        ...
        if result:
            ...
            await update.message.reply_text(f"{label}⚠️ LLMSelfbot didn't respond in time.")
        return
...
async def cmd_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if keyword == "check":
        ...
        if result:
            ...
            await update.message.reply_text(f"{label}⚠️ LLMSelfbot didn't respond in time. It may still be replying, check Discord.")
...
async def cmd_pfp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not url:
        # Check the command message itself, then the replied-to message
...
def _parse_platform_arg(context: ContextTypes.DEFAULT_TYPE):
    """Return 'snap'/'snapN', 'discord', or None from the first command arg."""
...
async def _error_notification_loop(app):
    ...
    while True:
        ...
        for i in range(1, NUM_SNAP_ACCOUNTS + 1):
            targets.append("snap" if i == 1 else f"snap{i}")
...
def main():
    ...
    if NUM_SNAP_ACCOUNTS > 1:
        ...
        print("[TG Controller] Snapchat  : enabled, use /account snap")
    else:
        print("[TG Controller] Snapchat  : disabled")
    print("[TG Controller] IPC files : ../config/tg_commands_N.json (per account) + ../ipc/tg_commands_snap.json")
    ...
    async def _post_init(application):
        ...
        # Register the command menu so Telegram shows the "/" command list.
        try:
