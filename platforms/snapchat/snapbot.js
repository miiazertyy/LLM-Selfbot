...
// Downloaded media (snap screenshots, voice clips) lands in a temp folder inside
// config/ so it's all in one place and easy to clear. The Python bridge deletes
// each file right after reading it.
const TEMP_DIR = path.resolve(__dirname, "../../config/temp");

// Build a temp-media filename that includes WHO sent it, so saved snaps/voices/
// images are recognisable (e.g. snap_Alexander_1781051313957.jpg) instead of an
// opaque chat UUID. The name is sanitised to a Windows-safe slug; we keep the
// timestamp to keep every file unique.
...
// Whether to print extra snap/vision debug. Treat "false"/"0"/"" as OFF so a
// disabled value in .env doesn't accidentally turn it on.
const SNAP_DEBUG = ["1", "true", "yes", "on"].includes(
  String(process.env.SNAP_DEBUG || "").toLowerCase()
);

// Scroll the (virtualized) chat list so older conversations below the fold get
// loaded and enumerated too. On by default; set SNAP_SCROLL_CHAT_LIST=false to
// only ever look at the chats already visible at the top.
const SCROLL_CHAT_LIST = String(process.env.SNAP_SCROLL_CHAT_LIST || "true").toLowerCase() !== "false";
// Hard cap on scroll steps so a huge list can't make one poll take forever.
...
export default class SnapBot {
  ...
  async launchSnapchat(obj, cookiefile) {
    try {
      ...
      // Load saved cookies if we have them. A missing cookie file on the very
      // first run is normal — not an error — so we only warn on a real failure.
      let cookiesLoaded = false;
      if (cookiefile) {
        const cookiePath = path.join(__dirname, `${cookiefile}-cookies.json`);
      ...
      // Auto-dismiss "Leave site? Changes you may not be saved" (beforeunload)
      // dialogs so a pending compose state can never freeze a navigation.
      ...
      // With saved cookies, go straight to the web app so the existing session
      // is detected (otherwise Snapchat shows the login page and we'd log in
      // again unnecessarily). Without cookies, start at the login page.
      ...
      // Snapchat shows a cookie-consent modal on first load that overlays and
      // blocks the login form — dismiss it before doing anything else.
  ...
  async login(credentials) {
    ...
    try {
      // If we're not already on the dedicated login page (e.g. stale cookies
      // bounced us to the marketing landing page snapchat.com/), navigate there
      // so the accountIdentifier field is reliably present.
      ...
      // accounts.snapchat.com/v2/login renders quickly – small grace period
      await delay(2000);

      // Clear the cookie-consent modal if it's covering the form.
      await this.acceptCookies();

      // On accounts.snapchat.com the username field uses name="accountIdentifier"
      // or id="accountIdentifier". The www.snapchat.com web app (older) used
      // placeholder text. Try all known variants.
      ...
      if (!usernameSelector) {
        // Dump the page HTML and a screenshot so we can debug the selector
        try {
   ...
   * verification step ("Check your email and click the link we sent you").
   * This method detects that screen and waits (up to `timeoutMs`) for the
   * user to click the link in their inbox, at which point the browser will
   * automatically land on web.snapchat.com.
   *
  ...
  /**
   * If there's an incoming voice/video call overlay, click "Not Now" to decline
   * it and return { name } of the caller (parsed from the "X is calling…" text).
   * Returns null when there's no incoming call.
   */
  async declineIncomingCall() {
    try {
      // 1) Detect an incoming call + the caller name WITHOUT clicking yet, so we
      //    can grab a snapshot of the call screen before it disappears.
      const info = await this.page.evaluate(() => {
        ...
        // Pull the caller name from a SMALL "<name> is calling…" element — never a
        // big container (which would slurp the whole sidebar). Match the shortest.
        ...
        for (const el of document.querySelectorAll("h1, h2, h3, span, div, p, b, strong")) {
          ...
          if (!txt || txt.length > 48) continue; // skip containers
      ...
      // 2) Snapshot the call screen (a video call shows the caller's camera)
      //    BEFORE declining — saved as call_<name>_<timestamp>.jpg in config/temp.
      let callShot = null;
      try {
  ...
  /**
   * Put `obj.path` into the snap composer so it sends as a real Snap (tap-to-view)
   * rather than a chat picture. Flow: open the camera composer, then UPLOAD the
   * image into the composer's file input (instead of taking a webcam photo), then
   * add an optional caption. The caller then runs send(name) for the recipient.
   */
  async captureSnap(obj) {
    if (!obj || !obj.path) return false;
    try {
      // 1) Open the camera / snap composer. If this fails we must NOT proceed —
      //    uploading with no composer open would dump the image into the open
      //    chat as a plain picture, which is exactly what we're avoiding.
      ...
      // 2) Upload the picture into the composer's file input. This is the bit
      //    that actually puts YOUR image into the snap (the old img.src swap
      //    didn't change the frame Snapchat sends).
      let uploaded = false;
      ...
      // 2b) THE snap-vs-picture check. A real Snap shows a near-fullscreen
      //     preview; if the image instead landed in the chat input as a small
      //     attachment thumbnail, this is a plain PICTURE — cancel it so we never
      //     send it, and report failure.
      ...
      // 3) Optional caption (best-effort — composer's text overlay).
      if (obj.caption) {
        try {
          ...
          for (const sel of ['textarea[aria-label="Caption Input"]', "textarea.B9QiX", "textarea"]) {
            ...
            if (ta) { await ta.type(obj.caption, { delay: 80 }); break; }
          }
  ...
  /**
   * True only when the real, near-fullscreen Snap composer is showing (a large
   * image/canvas/video preview covering most of the viewport). A chat picture
   * attachment is a tiny thumbnail in the input bar, so this returns false for it
   * — that's how we tell a Snap apart from a plain picture.
   */
  ...
  /**
   * Remove a pending image attachment from the chat input (so a mis-uploaded
   * picture is never sent and doesn't block the next text reply). Best-effort:
   ...
   *
   * Uses the camera-preview injection path (captureSnap) + the recipient picker
   * (send), then navigates back to the chat list so the polling loop recovers
   * regardless of whether the send succeeded.
   *
   * Note: this drives Snapchat's fragile camera UI — selectors can shift between
   * builds. Failures are non-fatal; the text reply is sent separately.
   */
  async sendImageSnap({ name, imagePath, caption, chat }) {
    ...
    // Real Snap (tap-to-view), NOT a chat picture: open the camera composer,
    // upload the image INTO it, then pick the recipient and send. Uploading into
    // a chat instead would send a plain media message — which is what we're
    // fixing here.
    const composed = await this.captureSnap({ path: imagePath, caption: caption || "", chat });
    if (!composed) {
      // Composer never opened — nothing was sent. Do NOT navigate (a page reload
      // here is what triggered the re-login / "Not now" flow), and surface the
      // failure so the runner logs it accurately instead of "✅ Picture sent".
    ...
    try {
      ...
      // Only AFTER we entered the composer do we navigate back to recover the
      // app view (the composer/recipient panel would otherwise block polling).
      try {
  ...
  /**
   * Open the "Added me" / friend-requests panel (the person+ icon top-left, with
   * the pending-count badge) and accept everyone by clicking each Add button.
   * Returns how many it added. Best-effort — Snapchat's classes shift between
   * builds, so it matches the button by its text ("Add"/"Accept").
   */
  async addPendingFriends(maxAdds = 5) {
    // Only open when there's a numeric pending-count badge top-left (the person+
    // icon). No badge = no requests, so we skip — no panel churn, no chat block.
    ...
    // Accept only a SMALL batch this pass so chatting isn't blocked for long —
    // the loop calls this each cycle, draining the backlog gradually.
    let added = 0;
   ...
   * can avoid opening every conversation on every poll.
   *
   * Unread heuristic (Snapchat marks unread conversations visually bold):
   *   • the title or status text is bold (computed font-weight >= 600), OR
   *   • the status text reads like a new/incoming item ("New Snap", "New Chat",
   *     "Received", "Sent you a snap", etc.)
   * "Read"-looking states (Opened / Delivered / Sent / Sending) clear the flag.
   * When in doubt we leave unread=false so we don't reopen already-read chats;
   * if this is too aggressive on your build, set SNAP_ONLY_UNREAD=false.
   */
  async listRecipientsWithUnread() {
    await this.page.waitForSelector(
      "div.ReactVirtualized__Grid__innerScrollContainer"
    );
    // The chat list is virtualized — only the visible rows exist in the DOM. To
    // see older conversations we scroll the list top→bottom, accumulating rows by
    // id across scroll steps (each step renders a fresh window), then return to
    // the top so the next poll starts clean.
   ...
   * Returns true if the chat was found in the list and handled, false otherwise
   * (e.g. it scrolled out of the visible list) so callers can detect a silent
   * miss instead of assuming the send succeeded.
   *
   *   message: ""        → just open the chat (used for reading); never types
   *   message: "text"    → open (unless alreadyOpen) then type + Enter
   *   message: ["a","b"] → send each in turn
   */
  /**
  ...
  async sendMessage(obj) {
    ...
    // If the chat is an older one below the fold, scroll the list to bring it
    // into the rendered window first (otherwise it won't be among the listitems).
    if (obj.chat) {
    ...
    for (const listItem of lists) {
      ...
      // If the conversation is already open, do NOT re-click the title — a second
      // click can toggle/close it, which is why the chat input sometimes vanished.
      ...
      for (const msg of messages) {
        ...
        if (msgBox) {
          await msgBox.type(`${msg}`, { delay: 80 });
          await this.page.keyboard.press("Enter");
        } else {
    ...
    if (!matched) {
      // Expected when a chat scrolled out of the rendered list — not an error,
      // so it's debug-only noise.
   ...
   * the chat list — so list reordering during the typing delay can't make us
   * lose the chat. Returns true only if the message was actually sent.
   */
  async typeInOpenChat(chatId, message) {
    try {
      ...
      if (!box) return false;
      await box.type(`${message}`, { delay: 80 });
      await this.page.keyboard.press("Enter");
      return true;
    } catch {
      return false;
   ...
   * name into the search box, click the result whose title matches EXACTLY (to
   * avoid messaging the wrong person), type the message, then clear the search.
   * Returns true only if the message was actually typed and sent.
   */
  async sendMessageViaSearch(name, message) {
    ...
    try {
      ...
      await this.page.keyboard.up("Control");
      await this.page.keyboard.press("Backspace");
      await search.type(name, { delay: 60 });
      await delay(1800);

      // Click ONLY an exact name match — never a fuzzy/first result. Search
      // results don't use the chat-list DOM, so try several shapes.
  ...
  /**
   * Grab an UNANSWERED voice message from the currently-open conversation, fetch
   * its audio bytes (works for blob: URLs), save them to a file, and return
   * { path, mime } — or null if there's no new voice message.
   *
   * "Unanswered" = the voice <audio> comes after our last reply ("Me", red
   * border), so once we reply it won't be re-grabbed (restart-safe). A session
   * seen-set also de-dupes during the read→reply gap.
   */
   ...
   * post / spotlight / attached photo (NOT a tap-to-view snap). Screenshots the
   * largest content image in the unanswered tail (after our last reply) so the
   * AI can see it. Returns the saved file path, or null.
   */
   ...
   * Looks for the on-screen cue text ("Click to view" / "New Snap") inside the
   * conversation container so we only fire the (slower) open+screenshot when
   * there's actually something to view.
   */
   ...
   * the saved file path (or null). The screenshot is later sent to the vision
   * model so the AI can actually "see" the snap — same idea as Discord images.
   *
   * Detection is by on-screen cue ("Click to view" / "New Snap") scoped to the
   * open conversation (#cv-<chatId>) so we never click the chat-list buttons.
  ...
  async openAndScreenshotSnap(chatId, name) {
    try {
      ...
      // An OPENED snap takes over the screen, so look for a media element that
      // covers most of the viewport. This rejects the conversation view (whose
      // elements are small) — that's why we were screenshotting the chat before.
  ...
  async saveCookies(username) {
    try {
      ...
      const cookiePath = path.join(__dirname, `${username}-cookies.json`);
   ...
   * Snapchat reveals a small action toolbar when you hover a message — "Save in
   * Chat" is one of those buttons. We hover each text bubble, click the save
   * button (matched by label, EN/FR; the 2nd hover button as a fallback) and skip
   * any already saved so we never toggle a save back off. Best-effort: the
   * toolbar's classes shift between builds.
   *
   * Already-handled messages are remembered for the session, so after the first
   * pass each poll only hovers NEW messages (cheap, doesn't disrupt chatting).
   */
  async saveMessagesInChat(userId) {
    ...
    // as handled WITHOUT clicking. We can't tell if an old message is already
    // saved (the save icon is unlabeled), and clicking a saved one UNSAVES it.
    // So we only ever save messages that arrive *after* the baseline — those are
    // guaranteed unsaved, so clicking can only ever save (never toggle off).
    ...
    for (const bubble of bubbles) {
      try {
        ...
        // The toolbar is hover-state driven and renders a beat after the mouse
        // enters — a single fixed wait misses it ~1/4 of the time. So we re-hover
        // (resetting the pointer first so each hover fires a fresh mouse-enter)
        // and POLL for the button to appear, with a synthetic-event nudge.
        ...
        for (let attempt = 0; attempt < 3 && result === "none"; attempt++) {
          // Reset the pointer so the next hover re-triggers the toolbar even on a
          // retry over the same bubble.
          try { await this.page.mouse.move(8, 8); } catch { /* ignore */ }
          try { await bubble.hover(); } catch { /* ignore */ }
          // Backup nudge: fire pointer/mouse-over on the row in case the real
          // hover didn't wake React's handlers this time.
        ...
        if (result === "saved" || result === "saved2") {
          // "saved2" (the 2nd toolbar button) is the normal path on builds where
          // the save icon has no label — not an anomaly, so no per-message log.
          saved++;
          ...
          // skip the empty/transient misses that the retry already recovers from.
