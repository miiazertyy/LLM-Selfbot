...
Both the Discord adapter and the Snapchat bridge call generate_ai_response()
with a normalised IncomingMessage and get back a plain string reply.
All Discord-specific or Snapchat-specific logic lives in their own adapters.
"""
...
from utils.db import record_user_message, record_user_message_snap
...
def _france_time_str() -> str:
    try:
        ...
        dst_start = datetime(y, 3,  last_sunday(y, 3),  2, tzinfo=timezone.utc)
...
async def generate_ai_response(msg: IncomingMessage) -> Optional[str]:
    """
    Given a normalised IncomingMessage, return the AI's reply string (or None on failure).
    Handles memory, mood, language detection, history summarisation, and late-reply openers.
    This function is platform-agnostic — no Discord or Snapchat imports here.
    """
    uid = msg.user_id
    ...
    # ── Pictures / selfies (mirrors the Discord runner) ──────────────────────
    # If the user asks for a photo and we have an eligible one, tell the model it
    # IS sending a selfie right now and remember which file so the adapter can
    # actually send it. Stored on msg.extra so platform adapters stay in control.
    picture_to_send = None
    try:
        from utils.pictures import is_picture_request, pick_picture, pictures_enabled, selfie_instruction
    ...
    lang_entry["count"] += 1
    # Only (re)detect when there's enough text to tell. Short messages like "tg",
    # "ok", "lol" are ambiguous and were causing wrong-language replies (e.g.
    # answering "tg" in Russian). On a short first message we keep the default.
...
# ── Helpers used by platform adapters for manual / paused replies ────────────
# These let an adapter (e.g. the Snapchat bridge) record an incoming message
# without generating a reply (when the bot is paused), then later answer the
# accumulated unanswered messages on demand via a Telegram /reply command.

def add_user_message(user_id: str, channel_id: str, content: str):
    """Append a user turn to history WITHOUT generating a reply.

    Used when the bot is paused/ignoring so that /reply can still find and
    ...
    # Cap growth — a long pause must not let one chat's history grow without bound.
...
def pop_pending_user_messages(user_id: str, channel_id: str) -> str:
    ...
    Leaves the rest of the history intact so generate_ai_response() can re-append
    the combined content as a single user turn (avoiding a duplicate).
    """
    key = f"{user_id}-{channel_id}"
